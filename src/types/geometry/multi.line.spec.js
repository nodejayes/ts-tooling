...
describe('MultiLine Tests', () => {
    ...
    const indicator = 'MultiLineString';
    const srid = 4326;
    ...
    describe('[static Method]: FromJSON', () => {
        it('read without srid information', () => {
            const l = MultiLine.FromString(str);
            assert.isDefined(l);
            assert.deepEqual(l.coordinates, c1);
            assert.isUndefined(l.crs);
        });
        ...
        it('read with srid information', () => {
            const l = MultiLine.FromString(strSrid);
            assert.isDefined(l);
            assert.deepEqual(l.coordinates, c1);
        ...
        it('error on wrong type', () => {
            assert.throws(() => {
                MultiLine.FromString(strInvalidType);
            });
        });
    });
    describe('[static Method]: FromString', () => {
        it('read without srid information', () => {
            const l = MultiLine.FromJSON(JSON.parse(str));
            assert.isDefined(l);
            assert.deepEqual(l.coordinates, c1);
            assert.isUndefined(l.crs);
        });
        ...
        it('read with srid information', () => {
            const l = MultiLine.FromJSON(JSON.parse(strSrid));
            assert.isDefined(l);
            assert.deepEqual(l.coordinates, c1);
            assert.isDefined(l.crs);
            assert.equal(l.crs.GetSrId(), srid);
        ...
        it('error on wrong type', () => {
            assert.throws(() => {
                MultiLine.FromJSON(JSON.parse(strInvalidType));
            });
        });
    });
