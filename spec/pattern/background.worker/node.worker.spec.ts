...
import {NodeBackgroundWorker} from '../../../src/ts-tooling';
...
describe('NodeBackgroundWorker Tests', () => {
    it('run BackgroundWorker with external File', (done) => {
        const worker = new NodeBackgroundWorker(WORKER_1_TYPESCRIPT);
    ...
    it('run multiple Workers', (done) => {
        let counter = 1;
        const worker1 = new NodeBackgroundWorker(WORKER_1_TYPESCRIPT);
    ...
    it('run javascript worker', (done) => {
        const worker = new NodeBackgroundWorker(WORKER_2_JAVASCRIPT);
        worker.OnFinish.subscribe((d) => {
            assert.equal(d, 2.6525285981219103e+32);
            done();
        });
    ...
    it('throws Error when empty worker path was given', () => {
        let worker = new NodeBackgroundWorker('');
        assert.throws(() => {
            worker.Run();
        }, 'missing DoWork Path/File ');
        worker = new NodeBackgroundWorker(NOT_EXISTS_WORKER_FILE);
        assert.throws(() => {
            worker.Run();
        }, 'missing DoWork Path/File ' + NOT_EXISTS_WORKER_FILE);
    });
    it('throws Error when has no Typescript or Javascript endings', () => {
        let worker = new NodeBackgroundWorker(TEST_BASH_SCRIPT);
        assert.throws(() => {
            worker.Run();
